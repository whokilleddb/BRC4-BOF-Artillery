...
DWORD SetPrivilege(HANDLE hTokenArg, LPCSTR lpszPrivilege, BOOL bEnablePrivilege) {
    HANDLE hToken = NULL;
    TOKEN_PRIVILEGES tp;
    LUID luid;
    ...
    if ( hTokenArg ) {
        ...
        if (FALSE == Advapi32$OpenProcessToken(Kernel32$GetCurrentProcess(), TOKEN_ADJUST_PRIVILEGES | TOKEN_QUERY, &hToken)) {
            BadgerDispatch(g_dispatch, "[-] Error OpenProcessToken: (%lu)\n", Kernel32$GetLastError()); 
            goto SetPrivilege_end;
        }
    }
    if ( FALSE == Advapi32$LookupPrivilegeValueA(NULL, lpszPrivilege, &luid)) {
    ...
    if ( FALSE == Advapi32$AdjustTokenPrivileges(hToken, FALSE, &tp, sizeof(TOKEN_PRIVILEGES), (PTOKEN_PRIVILEGES) NULL, (PDWORD) NULL) ) {
...
void coffee( char ** argv, int argc, WCHAR** dispatch) {
	...
	if (ERROR_SUCCESS != dwErrorCode) {
		BadgerDispatch(dispatch, "[-] Set privilege failed: %lu\n", Kernel32$GetLastError());
		goto main_end;
	}
